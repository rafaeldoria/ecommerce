    ...
    @if ($isFormOpen)
        <form class="space-y-5 rounded-lg border border-zinc-800 bg-zinc-900/70 p-5" wire:submit="save">
            <h2 class="text-lg font-semibold text-white">
                {{ $editingGameId === null ? __('admin.games.create_title') : __('admin.games.edit_title') }}
            </h2>

            <div>
                <label class="text-sm font-medium text-zinc-200" for="game-name">{{ __('admin.games.name_label') }}</label>
                <input
                    id="game-name"
                    ...
                    type="text"
                    wire:model="name"
                    placeholder="{{ __('admin.games.name_placeholder') }}"
                    autocomplete="off"
                >
                @error('name')
            ...
            <div class="flex flex-wrap gap-3">
                ...
                >
                    <span wire:loading.remove wire:target="save">
                        {{ $editingGameId === null ? __('admin.shared.create') : __('admin.shared.update') }}
                    </span>
    ...
    <div class="overflow-x-auto rounded-lg border border-zinc-800 bg-zinc-900/70">
        <table class="min-w-full divide-y divide-zinc-800">
            ...
            <tbody class="divide-y divide-zinc-800">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap justify-end gap-2">
                                <button class="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-100 transition hover:border-zinc-500 hover:bg-zinc-800" type="button" wire:click="edit({{ $game->id }})">
                                    {{ __('admin.shared.edit') }}
                                </button>

                                @if ($confirmingDeleteGameId === $game->id)
                                    ...
                                    <button class="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-100 transition hover:border-zinc-500 hover:bg-zinc-800" type="button" wire:click="$set('confirmingDeleteGameId', null)">
                                        {{ __('admin.shared.cancel_delete') }}
                                    </button>
                                @else
                                    <button class="rounded-lg border border-red-500/60 px-3 py-2 text-sm font-medium text-red-200 transition hover:border-red-400 hover:bg-red-950/40" type="button" wire:click="confirmDelete({{ $game->id }})">
                ...
                @empty
                    <tr>
                        <td class="px-6 py-8 text-zinc-400" colspan="3">{{ __('admin.games.empty_state') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
