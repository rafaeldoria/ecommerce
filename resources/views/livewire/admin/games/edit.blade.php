<section class="space-y-6">
    <div>
        <a class="text-sm font-medium text-emerald-300 transition hover:text-emerald-200" href="{{ route('admin.games.index') }}">
            {{ __('admin.shared.back_to_index') }}
        </a>
        <h1 class="mt-3 text-3xl font-semibold text-white">{{ __('admin.games.edit_title') }}</h1>
        <p class="mt-3 text-zinc-300">#{{ $game->id }} · {{ $game->name }}</p>
    </div>
    ...
    <form class="space-y-5 rounded-lg border border-zinc-800 bg-zinc-900/70 p-5" wire:submit="save">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a class="inline-flex min-h-11 items-center justify-center rounded-lg border border-zinc-700 px-4 py-2 text-sm font-semibold text-zinc-100 transition hover:border-zinc-500 hover:bg-zinc-800" href="{{ route('admin.games.index') }}">
